...
# count(컬럼명) : 지정된 컬럼 값 중 null인 컬럼을 제외한 행의 계수
select
    *
from
    tbl_category;
...
### group by 사용 시 주의 사항
# 1. NULL도 별도 그룹으로 묶임
# 2. select 절에는 group by 기준이된 컬럼 + 그룹함수만 작성 가능

select
    ref_category_code
    # category_code 그룹화 되지 안흥ㄴ 컬럼으로 인한 오류
from
    tbl_category
group by
    ref_category_code;
