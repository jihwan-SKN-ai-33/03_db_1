...
FROM
    tbl_menu
WHERE
    menu_price < 9000; # 1, 2, 3, 4, 10, 12, 13, 17, 21
...
WHERE
    ...
    b.menu_code IS NULL; # ** 교집합된 a코드를 빼겠다
...
# 카테고리 테이블에서
# ref_category_code 값이 1인 카테고리 코드를 찾다
# 메뉴 테이블에서
# 같은 카테고리의 메뉴를 모두 조회
...
# 1. 4(5,6,7,...)번 카테고리 메뉴 중 가장 비싼 메뉴 가격 조회
select
    max(menu_price)
from
    tbl_menu
