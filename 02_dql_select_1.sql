...
# ===========================================
# order by 절
# - 조회 결과(ResultSet)의 정럴 순서를 지정하는 구문
# - 특정 컬럼을 기준으로 삼아서 오른차순(ASC), 내림차순(DESC) 지정
# - 기준으로 삼은 컬럼이 여러개 존재하면 그룹화된 정렬이 수행된다.
...
# select절에 작성되지 않은 컬럼도 정렬기준이 될 수 있다
select
    menu_name
from
    tbl_menu
...
# 기준으로 삼은 컬럼이 여러개 존재하면 그룹화된 정렬이 수행
# 메뉴 테이블에서
# 메뉴명, 카테고리코드, 가격을 조회
...
# 메뉴 테이블에서 가격이 10,000원 이상, 20.00원 이하인
# 메뉴의 메뉴명, 가격, 카테고리 코드를
# 가격 내림차순으로 조회
select
    menu_name,
...
# or
#     category_code = 10
    category_code not in(4, 6, 10) # 4,5,10 제외
order by
    category_code asc;
...
# 카테고리 테이블에서 ref_category_coder가 NULL인 행 조회
select
    *
from
    tbl_category
where
    # ref_category_code 칼럼 안에 null 이란 값이 있으면 True
    # -> null은 값(data)가 아님
    # 컬럼이 비었음을 나타내는 기호
...
# limit offset, n : offset 행 만큼 건더뛰고 n행 만큼 조회
